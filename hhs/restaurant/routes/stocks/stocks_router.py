...
@router.get("/list",response_model = stocks_schema.StocksList,summary="레시피 목록 전체 조회")
...
@router.get("/detail/{stock_id}",response_model = stocks_schema.Stocks,summary="특정 레시피 상세 조회")
...
@router.put("/update",status_code=status.HTTP_204_NO_CONTENT,summary="레시피 수정")
def update_stocks(_stock_update:stocks_schema.StocksUpdate,db:Session = Depends(get_db)):
        
    db_stock = stocks_crud.get_stock(db,stock_id=_stock_update)
...
def delete_stocks(_stock_delete:stocks_schema.StocksDelete,db:Session=Depends(get_db)):
    ...
    db_stock = stocks_crud.get_stock(db,stock_id=_stock_delete)
